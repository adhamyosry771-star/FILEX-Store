...
import { Bell, ArrowRight, ArrowLeft, Mail, Info, CheckCircle, AlertTriangle } from 'lucide-react';
...
const NotificationsPage: React.FC<NotificationsPageProps> = ({ notifications, onBack, lang, onMarkAsRead, onMarkAllRead }) => {
    ...
    // Auto mark as read when clicking a specific notification (can be expanded to show full detail modal)
                ...
                {filteredNotifications.length > 0 ? (
                    filteredNotifications.map(notification => (
                        <div 
                            ...
                            className={`relative bg-slate-800/50 border rounded-2xl p-4 transition-all cursor-pointer ${notification.read ? 'border-slate-700/50 opacity-80' : 'border-teal-500/30 bg-slate-800 shadow-lg'}`}
                        >
                            {!notification.read && (
                                <div className="absolute top-4 left-4 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.5)]"></div>
                            )}
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-[10px] text-slate-600 mt-2 text-left ltr-text font-mono">
